...
function init() {
    ...
    document.querySelector('#remoteVideo').addEventListener('click', changeVideoUI);
    document.querySelector('#localVideo').addEventListener('click', changeVideoUI);
...
async function createRoom() {
    ...
    document.querySelector(
        '#currentRoom').innerText = `当前房间号是 ${roomId}`;
    ...
    calleeCanMonitorFunc(async () => {
        ...
        if (data != null && data != undefined && data.length > 0) {
            ...
            for (const x of data) {
                await peerConnection.addIceCandidate(new RTCIceCandidate(x.candidate));
            }
        }
...
function changeVideoUI(){
    let localClass = document.querySelector("#localVideo").className;
    if(localClass=='localStyle') {
        document.querySelector("#localVideo").className = 'remoteStyle'
        document.querySelector("#remoteVideo").className = 'localStyle'
    }else {
        document.querySelector("#localVideo").className = 'localStyle'
        document.querySelector("#remoteVideo").className = 'remoteStyle'
    }
}
